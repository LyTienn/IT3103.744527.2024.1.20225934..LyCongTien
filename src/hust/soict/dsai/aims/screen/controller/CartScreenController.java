...
public class CartScreenController {
    ...
    @FXML
    private TableColumn<MediaTienLC, Float> colMediaCost;

    @FXML
    private TableColumn<MediaTienLC, String> colMediaTitle;

    @FXML
    private TableColumn<MediaTienLC, String> colMediacategory;

    @FXML
    private TableView<MediaTienLC> tblMedia;
    ...
    void btnPlayPressed(ActionEvent event) {
        MediaTienLC media = tblMedia.getSelectionModel().getSelectedItem();
        Alert alert;
        try {
    ...
    void btnRemovePressed(ActionEvent event) {
        ...
        costLabel.setText(cart.totalCostTienLC() + " $");
    }

    public CartScreenController(CartTienLC cart) {
        super();
        this.cart = cart;
    }
    ...
    void initialize() {
        colMediaTitle.setCellValueFactory(
            new PropertyValueFactory<MediaTienLC, String>("title")
        );
        colMediacategory.setCellValueFactory(
            new PropertyValueFactory<MediaTienLC, String>("category")
        );
        colMediaCost.setCellValueFactory(
            new PropertyValueFactory<MediaTienLC, Float>("cost")
        );
        ...
        tblMedia.getSelectionModel().selectedItemProperty().addListener(
            new ChangeListener<MediaTienLC>() {

                @Override
                public void changed(ObservableValue<? extends MediaTienLC> observable, MediaTienLC oldValue, MediaTienLC newValue) {
                    if (newValue != null) {
                        updateButtonBar(newValue);
                    }
                }

                private void updateButtonBar(MediaTienLC media) {
        ...
        tfFilter.textProperty().addListener(
            ...
            private void showFilteredMedia(String keyword) {
                FilteredList<MediaTienLC> filteredList = new FilteredList<>(cart.getItemsOrdered());
